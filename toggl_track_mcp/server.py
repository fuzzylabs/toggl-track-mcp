...
async def get_current_user() -> Dict[str, Any]:
    ...
    try:
        user = await toggl_client.get_current_user()
        return {
...
async def get_current_time_entry() -> Dict[str, Any]:
    ...
    try:
        entry = await toggl_client.get_current_time_entry()
        if not entry:
            return {"message": "No time entry is currently running"}
        
        duration = toggl_client.calculate_duration(entry)
        duration_formatted = toggl_client.format_duration(duration)
...
async def list_time_entries(
    ...
    """
    try:
        entries = await toggl_client.get_time_entries(start_date, end_date)
        ...
        for entry in entries:
            ...
            if billable is not None and entry.billable != billable:
                continue
            if description_contains and description_contains.lower() not in entry.description.lower():
                continue
        ...
        total_formatted = toggl_client.format_duration(total_duration)
...
async def get_time_entry_details(entry_id: int) -> Dict[str, Any]:
    ...
    """
    try:
        entry = await toggl_client.get_time_entry(entry_id)
        duration = toggl_client.calculate_duration(entry)
...
async def list_projects() -> Dict[str, Any]:
    ...
    try:
        projects = await toggl_client.get_projects()
...
async def list_clients() -> Dict[str, Any]:
    """Get all clients."""
    try:
        clients = await toggl_client.get_clients()
...
async def list_workspaces() -> Dict[str, Any]:
    ...
    try:
        workspaces = await toggl_client.get_workspaces()
...
async def list_tags() -> Dict[str, Any]:
    ...
    try:
        tags = await toggl_client.get_tags()
...
async def get_time_summary(
    ...
    try:
        ...
        # Format breakdowns
        def format_breakdown(breakdown_dict):
            return [
                {
                    "name": name,
                    "duration": data["duration"],
                    "duration_formatted": toggl_client.format_duration(data["duration"]),
                    "entries_count": data["count"]
                }
                for name, data in sorted(breakdown_dict.items(), key=lambda x: x[1]["duration"], reverse=True)
            ]
        
        return {
            "summary": {
                "total_duration": total_duration,
                "total_duration_formatted": toggl_client.format_duration(total_duration),
                "billable_duration": billable_duration,
                "billable_duration_formatted": toggl_client.format_duration(billable_duration),
                "non_billable_duration": total_duration - billable_duration,
                "non_billable_duration_formatted": toggl_client.format_duration(total_duration - billable_duration),
            ...
            "period_summary": f"Total time: {toggl_client.format_duration(total_duration)} ({len(filtered_entries)} entries)"
        }
    except TogglAPIError as e:
        return {"error": str(e)}
